...
class Forces:
        ...
        if self.collision:
            ...
            for shape_1, shape_2 in itertools.combinations(shapes, 2):
                if shape_1.shape == shape_2.shape == "Square":
                    ...
                    if rect1.colliderect(shape_2.x, shape_2.y, shape_2.size, shape_2.size):
                ...
                if shape_1.shape == shape_2.shape == "Circle":
                    ...
                    collide_speed = math.floor(np.sqrt(speed[0]**2 + speed[1]**2))
    ...
    def _collide(self, shape_1, shape_2):
        ...
        dot1 = (np.dot(shape_1.vel-shape_2.vel, shape_1.center-shape_2.center))
        dot2 = (np.dot(shape_2.vel-shape_1.vel, shape_2.center-shape_1.center))
        ...
        for surface in surfaces:
            for shape in shapes:
                ...
                if shape.shape == "Rectangle":
                    if surface.colliderect(shape.x, dy, shape.size, shape.size):
                        shape.y_vel *= -1
                    if surface.colliderect(dx, shape.y, shape.size, shape.size):
                        shape.x_vel *= -1
                if shape.shape == "Circle":
                    if surface.colliderect(shape.x, dy, shape.radius, shape.radius):
                        shape.y_vel *= -1
                        shape.is_bouncing  = False
                    if surface.colliderect(dx, shape.y, shape.radius, shape.radius):
                        if shape.y_vel < 0:
                            shape.x_vel *= -1
